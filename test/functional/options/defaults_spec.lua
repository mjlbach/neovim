...
describe("XDG-based defaults", function()
  ...
  it("&runtimepath data-dir matches stdpath('data') #9910", function()
    ...
    local rv = {}
    local expected = (
        iswin() and { [[\nvim-data\site]], [[\nvim-data\site\after]] } or { "/nvim/site", "/nvim/site/after" }
      )
  ...
  describe("with too long XDG variables", function()
    before_each(function()
      clear {
        env = {
          ...
          XDG_CONFIG_DIRS = (root_path .. ("/a"):rep(2048) .. env_sep .. root_path .. ("/b"):rep(2048) .. (
            env_sep .. root_path
            .. "/c"
          ):rep(512)),
          XDG_DATA_HOME = (root_path .. ("/X"):rep(4096)),
          XDG_DATA_DIRS = (root_path .. ("/A"):rep(2048) .. env_sep .. root_path .. ("/B"):rep(2048) .. (
            env_sep .. root_path
            .. "/C"
          ):rep(512)),
        },
    ...
    it("are correctly set", function()
      ...
      eq(
        ...
        (meths.get_option "runtimepath"):gsub("\\", "/")
      )
      ...
      eq(
        ".," .. root_path .. ("/X"):rep(4096) .. "/" .. data_dir .. "/backup",
        (meths.get_option("backupdir"):gsub("\\", "/"))
      )
      eq(root_path .. ("/X"):rep(4096) .. "/" .. data_dir .. "/swap//", (meths.get_option "directory"):gsub("\\", "/"))
      eq(root_path .. ("/X"):rep(4096) .. "/" .. data_dir .. "/undo", (meths.get_option "undodir"):gsub("\\", "/"))
      eq(root_path .. ("/X"):rep(4096) .. "/" .. data_dir .. "/view", (meths.get_option "viewdir"):gsub("\\", "/"))
    end)
  end)
  ...
  describe("with XDG variables that can be expanded", function()
    ...
    it("are not expanded", function()
      ...
      eq((".,$XDG_CONFIG_HOME/" .. data_dir .. "/backup"), meths.get_option("backupdir"):gsub("\\", "/"))
      eq(("$XDG_CONFIG_HOME/" .. data_dir .. "/swap//"), meths.get_option("directory"):gsub("\\", "/"))
      eq(("$XDG_CONFIG_HOME/" .. data_dir .. "/undo"), meths.get_option("undodir"):gsub("\\", "/"))
      eq(("$XDG_CONFIG_HOME/" .. data_dir .. "/view"), meths.get_option("viewdir"):gsub("\\", "/"))
  ...
  describe("with commas", function()
    ...
    it("are escaped properly", function()
      ...
      eq(".,\\,=\\,=\\," .. path_sep .. data_dir .. "" .. path_sep .. "backup", meths.get_option "backupdir")
      eq(
        "\\,=\\,=\\," .. path_sep .. "" .. data_dir .. "" .. path_sep .. "swap" .. (path_sep):rep(2),
        meths.get_option "directory"
      )
      eq("\\,=\\,=\\," .. path_sep .. "" .. data_dir .. "" .. path_sep .. "undo", meths.get_option "undodir")
      eq("\\,=\\,=\\," .. path_sep .. "" .. data_dir .. "" .. path_sep .. "view", meths.get_option "viewdir")
    end)
  end)
end)
...
describe("stdpath()", function()
  ...
  context("returns a List", function()
    ...
    describe('with "config_dirs"', function()
      behaves_like_dir_list_env("handles XDG_CONFIG_DIRS with one path", "config_dirs", "XDG_CONFIG_DIRS", {
        alter_slashes "/home/docwhat/.config",
      }, {
        alter_slashes "/home/docwhat/.config/nvim",
      })

      behaves_like_dir_list_env("handles XDG_CONFIG_DIRS with two paths", "config_dirs", "XDG_CONFIG_DIRS", {
        alter_slashes "/home/docwhat/.config",
      ...
      behaves_like_dir_list_env("doesn't expand $VAR and $IBLES", "config_dirs", "XDG_CONFIG_DIRS", { "$HOME", "$TMP" }, {
    ...
    describe('with "data_dirs"', function()
      ...
      behaves_like_dir_list_env("knows XDG_DATA_DIRS with two paths", "data_dirs", "XDG_DATA_DIRS", {
        alter_slashes "/home/docwhat/.data",
      ...
      behaves_like_dir_list_env("doesn't expand $VAR and $IBLES", "data_dirs", "XDG_DATA_DIRS", { "$HOME", "$TMP" }, {
        alter_slashes "$HOME/nvim",
        alter_slashes "$TMP/nvim",
      })
