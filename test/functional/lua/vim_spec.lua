...
describe("lua stdlib", function()
  ...
  it("vim.deepcopy", function()
    ...
    eq(
      "Error executing lua: vim/shared.lua:0: Cannot deepcopy object of type thread",
      pcall_err(
        exec_lua,
        [[
  ...
  it("vim.tbl_extend", function()
    ...
    eq(
      'Error executing lua: vim/shared.lua:0: invalid "behavior": nil',
      pcall_err(
        exec_lua,
        [[
    ...
    eq(
      "Error executing lua: vim/shared.lua:0: wrong number of arguments (given 1, expected at least 3)",
      pcall_err(
        exec_lua,
        [[
    ...
    eq(
      "Error executing lua: vim/shared.lua:0: wrong number of arguments (given 2, expected at least 3)",
      pcall_err(
        exec_lua,
        [[
        return vim.tbl_extend("keep", {})
  ...
  it("vim.tbl_add_reverse_lookup", function()
    ...
    matches(
      '^Error executing lua: vim/shared%.lua:0: The reverse lookup found an existing value for "[1A]" while processing key "[1A]"$',
  ...
  it("vim.fn should error when calling API function", function()
    eq(
      "Error executing lua: vim.lua:0: Tried to call API function with vim.fn: use vim.api.nvim_get_current_line instead",
  ...
  it("vim.bo", function()
    ...
    matches(
      "^Error executing lua: .*: Invalid option name: 'nosuchopt'$",
      pcall_err(exec_lua, "return vim.bo.nosuchopt")
    )
    matches("^Error executing lua: .*: Expected lua string$", pcall_err(exec_lua, "return vim.bo[0][0].autoread"))
  end)
  ...
  it("vim.wo", function()
    ...
    matches("^Error executing lua: .*: Invalid option name: 'notanopt'$", pcall_err(exec_lua, "return vim.wo.notanopt"))
    matches("^Error executing lua: .*: Expected lua string$", pcall_err(exec_lua, "return vim.wo[0][0].list"))
  ...
  describe("vim.wait", function()
    ...
    it("should not crash when callback errors", function()
      eq(
        { false, '[string "<nvim>"]:1: As Expected' },
        exec_lua [[
