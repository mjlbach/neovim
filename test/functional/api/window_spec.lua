...
local clear, nvim, curbuf, curbuf_contents, window, curwin, eq, neq, ok, feed, insert, eval =
  helpers.clear,
  helpers.nvim,
  helpers.curbuf,
...
describe("API/win", function()
  ...
  describe("{get,set}_height", function()
    it("works", function()
      ...
      nvim("set_current_win", nvim("list_wins")[2])
      nvim("command", "split")
      eq(window("get_height", nvim("list_wins")[2]), math.floor(window("get_height", nvim("list_wins")[1]) / 2))
  ...
  describe("{get,set}_width", function()
    it("works", function()
      ...
      nvim("set_current_win", nvim("list_wins")[2])
      nvim("command", "vsplit")
      eq(window("get_width", nvim("list_wins")[2]), math.floor(window("get_width", nvim("list_wins")[1]) / 2))
  ...
  describe("{get,set}_option", function()
    it("works", function()
      ...
      command "enew"
      eq("Failed to get value for option 'statusline'", pcall_err(curwin, "get_option", "statusline"))
  ...
  describe("close", function()
    ...
    it("handles changed buffer", function()
      ...
      command "new"
      local newwin = meths.get_current_win()
      eq("Vim:E37: No write since last change (add ! to override)", pcall_err(meths.win_close, oldwin, false))
    ...
    it("in cmdline-window #9767", function()
      ...
      eq("E11: Invalid in command-line window; <CR> executes, CTRL-C quits", pcall_err(meths.win_close, oldwin, true))
