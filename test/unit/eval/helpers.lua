...
local eval = cimport("./src/nvim/eval.h", "./src/nvim/eval/typval.h", "./src/nvim/hashtab.h", "./src/nvim/memory.h")
...
local function list(...)
  return populate_list(ffi.gc(eval.tv_list_alloc(select("#", ...)), eval.tv_list_unref), { ... }, {})
end
...
local function tbl2callback(tbl)
  ...
  elseif tbl.type == "fref" then
    ret = ffi.new("Callback[1]", { { type = eval.kCallbackFuncref, data = { funcref = eval.xstrdup(tbl.fref) } } })
...
local function dict_watchers(d)
  ...
  local key_patterns = {}
  while q ~= h do
    local qitem = ffi.cast("DictWatcher *", ffi.cast("char *", q) - ffi.offsetof("DictWatcher", "node"))
    ret[#ret + 1] = {
