...
local eq, eval, expect, source = helpers.eq, helpers.eval, helpers.expect, helpers.source
...
describe(":recover", function()
  ...
  it("fails if given a non-existent swapfile", function()
    ...
    eq("Vim(recover):E305: No swap file found for " .. swapname, pcall_err(command, "recover " .. swapname)) -- Should not segfault. #2117
...
describe("swapfile detection", function()
  ...
  it("always show swapfile dialog #8840 #9027", function()
    ...
    feed ":silent edit %<CR>"
    screen2:expect {
      any = [[Found a swap file by the name ".*]] .. [[Xtest_swapdialog_dir[/\].*]] .. testfile .. [[%.swp"]],
    }
    feed "e"
    screen2:expect(expected_no_dialog)
