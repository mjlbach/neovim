...
describe("luaeval()", function()
  ...
  describe("lua values", function()
    it("are successfully transformed", function()
      eq({ n = 1, f = 1.5, s = "string", l = { 4, 2 } }, funcs.luaeval '{n=1, f=1.5, s="string", l={4, 2}}')
  ...
  describe("strings", function()
    ...
    it("are successfully converted to special dictionaries from a list", function()
      command [[let l = luaeval('{"abc", "a\0b", "c\0d", "def"}')]]
      eq({ "abc", { _TYPE = {}, _VAL = { "a\nb" } }, { _TYPE = {}, _VAL = { "c\nd" } }, "def" }, meths.get_var "l")
  ...
  it("issues an error in some cases", function()
    eq(
      "Vim(call):E5100: Cannot convert given lua table: table should either have a sequence of positive integer keys or contain only string keys",
      exc_exec 'call luaeval("{1, foo=2}")'
    )

    startswith('Vim(call):E5107: Error loading lua [string "luaeval()"]:', exc_exec 'call luaeval("1, 2, 3")')
    startswith('Vim(call):E5108: Error executing lua [string "luaeval()"]:', exc_exec 'call luaeval("(nil)()")')
  end)
  ...
  it("errors out correctly when doing incorrect things in lua", function()
    -- Conversion errors
    eq(
      "Vim(call):E5108: Error executing lua [string \"luaeval()\"]:1: attempt to call field 'xxx_nonexistent_key_xxx' (a nil value)",
      exc_exec [[call luaeval("vim.xxx_nonexistent_key_xxx()")]]
    )
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: ERROR',
      exc_exec [[call luaeval("error('ERROR')")]]
    )
    eq("Vim(call):E5108: Error executing lua [NULL]", exc_exec [[call luaeval("error(nil)")]])
  end)

  it("does not leak memory when called with too long line", function()
    local s = ("x"):rep(65536)
    eq(
      "Vim(call):E5107: Error loading lua [string \"luaeval()\"]:1: unexpected symbol near ')'",
...
describe("v:lua", function()
  ...
  it("works in expressions", function()
    ...
    eq(
      "Vim:E5108: Error executing lua [string \"<nvim>\"]:0: attempt to call global 'nonexistent' (a nil value)",
  ...
  it("works in :call", function()
    ...
    eq(
      "Vim(call):E5108: Error executing lua [string \"<nvim>\"]:0: attempt to call global 'nonexistent' (a nil value)",
